...
class P2PNode:
    ...
    async def start_sync(
        ...
        """
        if self.is_server:
            # Receive peer's entry first to discover which folder they want to sync
            config_payload = await async_recv_config(reader)
            remote_data = yaml.safe_load(config_payload.decode("utf-8"))
            remote_entry = FolderEntry.model_validate(remote_data or {})
            ...
            if not local_entry:
                ...
                await async_send_error(writer, err_msg.encode("utf-8"))
                raise PeerAuthError(err_msg)
            try:
                ...
                await async_send_error(writer, str(exc).encode("utf-8"))
                raise

            own_payload = yaml.safe_dump(local_entry.model_dump(mode="json")).encode("utf-8")
            await async_send_config(writer, own_payload)
            ...
            exchange = ConfigExchange(own_entry=self.folder, own_device_id=self._own_device_id)
