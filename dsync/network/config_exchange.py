...
class ConfigExchange:
    ...
    async def _send_own_config(self, writer: asyncio.StreamWriter) -> None:
        ...
        config_yaml = yaml.safe_dump(
            self.own_config.model_dump(mode="json")
        ).encode("utf-8")
    ...
    def _validate_no_circular_conflict(self, peer_device_id: str, peer_config: FoldersConfig) -> None:
        ...
        source_backup_to_peer = self._get_backup_to_peer_paths(
            self.own_config, peer_device_id
        )

        # Get paths peer syncs TO source
        peer_backup_to_source = self._get_backup_to_peer_paths(
            peer_config, self.own_device_id
        )
    ...
    @staticmethod
    def _get_backup_to_peer_paths(
        config: FoldersConfig, target_peer_id: str
    ) -> set[str]:
        ...
        for entry in config.entries:
            if entry.mode == SyncMode.BACKUP_TO_PEER:
                # Check if this entry is configured to sync to the target peer
                if entry.devices is None or target_peer_id in entry.devices:
    ...
    @staticmethod
    def _get_mirror_paths(
        config: FoldersConfig, peer_id: str
    ) -> set[str]:
        ...
        for entry in config.entries:
            if entry.mode == SyncMode.MIRROR:
                # Check if this entry is configured to sync to the peer
                if entry.devices is None or peer_id in entry.devices:
                    paths.add(str(entry.path))
